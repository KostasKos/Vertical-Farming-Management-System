  ...
  const [users, setUsers] = useState([]); 
    ...
    useEffect(() => {
      ...
      const fetchRoomsAndSensors = async () => {
        try {
          const roomsResponse = await fetch(`http://127.0.0.1:5017/get_rooms_with_shelves_and_sensors?user_id=${userId}&role=${userRole}`);
...
useEffect(() => {
  const fetchUsers = async () => {
    try {
      const response = await fetch("http://127.0.0.1:5888/api/users");
...
const paginateRooms = (roomsToPaginate) => {
  ...
  const addRoom = async () => {
    ...
    if (userRole !== "Admin") {
      alert("You do not have permission to create rooms as user.");
      return;
    }
...
const filteredDevices = devices
  ...
  const [tempHumidity, setTempHumidity] = useState(humidity);
  
  const [tempCO2Level, setTempCO2Level] = useState(co2Level);
  
  const [tempLightIntensity, setTempLightIntensity] = useState(lightIntensity);
  
  const [tempPHLevel, setTempPHLevel] = useState(phLevel);
  
  const [tempECLevel, setTempECLevel] = useState(ecLevel);
...
<div className="flex-1">
  ...
  <Select value={selectedUserId} onValueChange={(value) => setNewUserId(value)}>
...
{/* Current Values Row */}
  ...
  <h2 className="text-xl font-bold mb-2">Live Environmental Conditions</h2>
  <div className="grid grid-cols-3 gap-4 col-span-2 mb-4 p-4 bg-gray-100 rounded-md">
    <div>
    <strong>Temperature:</strong> {temperatureControl ? `${temperature}°C` : "Off"}
  </div>
  <div>
    <strong>Humidity:</strong> {humidityControl ? `${humidity}%` : "Off"}
  </div>
  <div>
    <strong>CO2 Level:</strong> {co2Control ? `${co2Level} ppm` : "Off"}
  </div>
  <div>
    <strong>Light Intensity:</strong> {lightControl ? `${lightIntensity}%` : "Off"}
  </div>
  <div>
    <strong>pH Level:</strong> {phControl ? phLevel : "Off"}
  </div>
  <div>
    <strong>EC Level:</strong> {ecControl ? `${ecLevel} mS/cm` : "Off"}
  </div>
</div>
...
<Card>
  ...
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {temperatureControl ? `${temperature}°C` : "Off"}</span> */}
      <Switch
    ...
    <Slider
      className="h-1"
      value={[tempTemperature]} // Temporary value
      min={10}
      max={40}
      step={0.5}
      onValueChange={(value) => setTempTemperature(value[0])} // Update temp state
      disabled={!temperatureControl}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempTemperature}°C
    </div>
  </CardContent>
</Card>
...
<Card>
  <CardHeader>
    <CardTitle>Humidifier</CardTitle>
  </CardHeader>
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {humidityControl ? `${humidity}%` : "Off"}</span> */}
      <Switch
    ...
    <Slider
      className="h-1"
      value={[tempHumidity]} // Temporary value
      min={0}
      max={100}
      step={1}
      onValueChange={(value) => setTempHumidity(value[0])} // Update temp state
      disabled={!humidityControl}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempHumidity}%
    </div>
  </CardContent>
</Card>
...
<Card>
  <CardHeader>
    <CardTitle>CO2 Level Actuator</CardTitle>
  </CardHeader>
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {co2Control ? `${co2Level} ppm` : "Off"}</span> */}
      <Switch
        checked={co2Control}
    ...
    <Slider
      className="h-1"
      value={[tempCO2Level]} // Temporary value
      min={300}
      max={2000}
      step={10}
      onValueChange={(value) => setTempCO2Level(value[0])} // Update temp state
      disabled={!co2Control}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempCO2Level} ppm
    </div>
  </CardContent>
</Card>
...
<Card>
  <CardHeader>
    <CardTitle>UV Lamps</CardTitle>
  </CardHeader>
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {lightControl ? `${lightIntensity}%` : "Off"}</span> */}
      <Switch
    ...
    <Slider
      className="h-1"
      value={[tempLightIntensity]} // Temporary value
      min={0}
      max={100}
      step={1}
      onValueChange={(value) => setTempLightIntensity(value[0])} // Update temp state
      disabled={!lightControl}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempLightIntensity}%
    </div>
  </CardContent>
</Card>
...
<Card>
  <CardHeader>
    <CardTitle>pH Actuator</CardTitle>
  </CardHeader>
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {phControl ? phLevel : "Off"}</span> */}
      <Switch
        checked={phControl}
    ...
    <Slider
      className="h-1"
      value={[tempPHLevel]} // Temporary value
      min={0}
      max={14}
      step={0.1}
      onValueChange={(value) => setTempPHLevel(value[0])} // Update temp state
      disabled={!phControl}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempPHLevel}
    </div>
  </CardContent>
</Card>
...
<Card>
  <CardHeader>
    <CardTitle>EC Level Actuator</CardTitle>
  </CardHeader>
  <CardContent>
    <div className="flex items-center justify-between mb-2">
      {/* <span>Current: {ecControl ? `${ecLevel} mS/cm` : "Off"}</span> */}
      <Switch
        checked={ecControl}
    ...
    <Slider
      className="h-1"
      value={[tempECLevel]} // Temporary value
      min={0}
      max={5}
      step={0.1}
      onValueChange={(value) => setTempECLevel(value[0])} // Update temp state
      disabled={!ecControl}
    />
    <div className="text-sm text-gray-600">
      Adjusting to: {tempECLevel} mS/cm
    </div>
  </CardContent>
</Card>
  ...
  <Button
    ...
    onClick={() => {
      ...
      setCO2Level(tempCO2Level);
      setLightIntensity(tempLightIntensity);
      setPHLevel(tempPHLevel);
      setECLevel(tempECLevel);
      handleSetControls(selectedRoom, selectedShelf); // Additional logic
    }}
  >
    Set Controls
  </Button>
