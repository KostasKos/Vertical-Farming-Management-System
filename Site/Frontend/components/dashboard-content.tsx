...
useEffect(() => {
  const storedDashboardItems = localStorage.getItem('dashboardItems');
  if (storedDashboardItems) {
    // Parse the stored data and set the state
    setDashboardItems(JSON.parse(storedDashboardItems));
  }
...
if (selectedItemType === 'sensor') {
  ...
  if (sensorToAdd) {
    ...
    if (newItem && !dashboardItems.some(item => item.content.id === newItem!.content.id)) {
      const updatedDashboardItems = [...dashboardItems, newItem];
  
      // Save updated items to localStorage
      localStorage.setItem('dashboardItems', JSON.stringify(updatedDashboardItems));
  ...
  const removeItem = (itemId: string) => {
    ...
    // Update localStorage with the new list of dashboard items
